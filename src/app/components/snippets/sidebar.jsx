...
const Sidebar = () => {
  ...
  if (mode === 'folder') {
    ...
    if (currentFolderIndex !== -1) {
      currentFolderId = folders[currentFolderIndex].id;
    }
    ...
    for (let fIndex = 0; fIndex < folders.length; fIndex++) {
      ...
      if (sIndex !== -1) {
        ...
        currentFolderId = folders[fIndex].id;
        break;
      }
    }
  }
      ...
      <ul>
        {folders.map((folder) => (
          ...
          <div className={`px-2 py-1 ${
              pathname === `/snippets/folder/${folder.id}` ? 'bg-blue-200' : ''
            }`}>
            ...
            {folder.snippets.length === 0 ? (
                ...
                folder.snippets.map((snippet) => (
                  <li
                    key={snippet.id}
                    className={`mt-1 pa-2 ${
                      pathname === `/snippets/snippet/${snippet.id}` ? 'bg-green-200' : 'bg-transparent'
                    }`}
                  >
